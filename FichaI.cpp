...
FichaI::FichaI(int x, int y) {
 
    this->x1 = x; this->y1 = y;
    this->x2 = x; this->y2 = y+1;
    this->x3 = x; this->y3 = y+2;
    this->x4 = x; this->y4 = y+3;
    this->setTipo('I');
}

FichaI::FichaI(const FichaI& orig) {
}

FichaI::~FichaI() {
}

bool FichaI::mover(char input, Tablero& tab) {
    ...
    if (input == 'P'){
        if (this->rotacion == 0){
            if (not this->detectorColisiones(tab, -2, 2, -1, 1, 0, 0, 1, -1)){
